...
<div
	class="max-h-[none] overflow-hidden rounded-xl border border-slate-100 bg-white shadow-[0_5px_20px_#0001]"
>
	...
	<div class="flex flex-auto flex-col overflow-x-auto">
		<div class="flex flex-auto">
			<div class="flex flex-row">
				<pre
					class="select-none whitespace-nowrap text-right text-sm text-slate-400"
					aria-hidden="true">
					<code class="block p-[1em]">
						{#each lines as line}
							{line}<br />
						{/each}
					</code>
				</pre>
				<pre class="min-h-full flex-1 overflow-auto text-sm">{@html content}</pre>
			</div>
		</div>
	</div>
